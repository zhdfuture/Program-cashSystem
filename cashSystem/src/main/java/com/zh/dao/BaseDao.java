...
public class BaseDao {
    //连接数据库
    //DataSources较drivermanager.不会有频繁的创建和销毁
        ...
        if (dataSource == null) {
            synchronized(DataSource.class) {
                if (dataSource == null) {
                    ...
                    String port = "3306";
                    ((MysqlDataSource) dataSource).setUrl("jdbc:mysql://" + (host + ":" + port) + "/program?useSSL=false");
                    ((MysqlDataSource) dataSource).setUser("root");
                    ((MysqlDataSource) dataSource).setPassword("19913017AB");
                }
            }
        }
        return dataSource;
