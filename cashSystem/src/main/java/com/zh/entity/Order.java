...
public class Order {
  ...
  private LocalDateTime finish_time;
    ...
    //存放订单项的list
    public List<OrederItem> orederItemList=new ArrayList<>();@Override
    public String toString() {
        ...
        if (this.getOrder_status() == OrderStatus.OK) {
            sb.append("\t").append("【完成时间】：")
                    .append(this.timeToString(this.getFinish_time()));
        }
        sb.append("【订单明细】*************************************").append("\n");
        sb.append("\t编号   名称     数量     单位     单价（元）").append("\n");
        int i = 0;
        for (OrederItem orderItem : this.getOrederItemList()) {
